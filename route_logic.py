...
from utils import string_to_epoch
...
def format_leg_and_fetch_estimates(leg, bart_api_key):
...
def format_trip_with_estimates(trip, bart_api_key):
    ...
    pretty_leg_list = []
    for leg in trip['leg']:
        pretty_leg_list.append(format_leg_and_fetch_estimates(leg=leg, bart_api_key=bart_api_key))
...
def get_trips(req_dict, bart_api_key, route_cnt=1):
    ...
    if trips_resp.status_code == constants.HTTP_STATUS_OK:
        try:
            ...
            return {'response': json.loads(trips_resp.content)['root']}, 200
    else:
...
def get_trip_with_estimates(req_dict, bart_api_key):
    ...
    formatted_trip = format_trip_with_estimates(trip=trip, bart_api_key=bart_api_key)
